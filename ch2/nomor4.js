...
const getTotalPenjualan = (dataPenjualan) => {
  ...
  else if (!(dataPenjualan instanceof Array)) return "Error: Invalid data type";
  else if (dataPenjualan.length == 0) return "Error: Empty array";

  const totalPenjualan = dataPenjualan.reduce((previous, current) => {
    return previwous + current.totalTerjual;
  }, 0);
